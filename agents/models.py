"""
Pydantic v2 schemas and data definitions for Critical Panic Alert Agent.
...
class SystemTaskPayload(BaseModel):
    ...
    def sanitize_string_fields(cls, v: str) -> str:
        """Strip whitespace and prevent control characters in string fields."""
    ...
    def validate_metric_finite(cls, v: float) -> float:
        """Ensure metric values are finite (not NaN or Infinity)."""
        if math.isnan(v) or math.isinf(v):
            raise ValueError("Metric value must be a finite number (not NaN or Infinity)")
        return v
...
class ConsensusDossier(BaseModel):
    dossier_id: str
    system_slug: str = "critical-panic-alert-agent"
